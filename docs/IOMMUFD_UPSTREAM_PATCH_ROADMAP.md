...
VFIO PCI는 device BAR/MMIO 범위를 DMA-BUF로 export하고, IOMMUFD는 이를
`IOMMU_IOAS_MAP_FILE`로 IOAS에 넣는다. 이 경로는 GPU를 vendor driver에 둔 채
CUDA allocation을 export하는 문제와는 다르지만, VFIO-bound GPU BAR를 NVMe와
같은 IOAS에 배치하는 UPCIE PR의 upstream 기반이다.
...
따라서 다음 상세 분석은 merge된 `MAP_FILE`/VFIO DMA-BUF를 먼저 기준선으로
정리하고, no-IOMMU cdev v10과 `guest_memfd` RFC를 각각 비교하는 순서가 가장
효율적이다.
