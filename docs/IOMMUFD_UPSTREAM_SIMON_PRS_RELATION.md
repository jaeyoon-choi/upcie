...
검토 기준일은 2026-07-13이며, upstream 논의 상태는 2026-07-15에
갱신했다. PR의 의존 관계는 다음과 같다.
...
| VFIO PCI MMIO DMA-BUF exporter | Jason Gunthorpe, Vivek Kasireddy, Leon Romanovsky | v6.19 merge | [Vivek v3](https://lore.kernel.org/all/20250307052248.405803-1-vivek.kasireddy@intel.com/), [merged exporter](https://github.com/torvalds/linux/commit/5d74781ebc86c5fa9e9d6934024c505412de9b52) | #38의 직접 기반 | GPU BAR/MMIO range를 revocable DMA-BUF fd로 만드는 공급자다. 이 기능이 없으면 #38이 IOMMUFD에 넘길 fd 자체가 없다. device close/reset과 range lifetime은 이 계층의 책임이다. |
| IOMMUFD의 VFIO DMA-BUF `IOAS_MAP_FILE` importer | Jason Gunthorpe | v6.19 merge | [DMA-BUF importer patch](https://lore.kernel.org/all/8-v2-b2c110338e3f+5c2-iommufd_dmabuf_jgg@nvidia.com/) | #38의 직접 기반 | exporter가 만든 fd를 받아 동일 IOAS에 IOVA를 만드는 반대쪽 절반이다. exporter와 importer를 분리해야 revoke, unmap과 fd/object ownership을 정확히 검토할 수 있다. 후속 revoke/pin 계열(Leon Romanovsky, `dma_buf_pin()` v7)이 merge됐고, 6.21 pull에 dma_resv 잠금과 detach 수명 수정이 이어지고 있어 활발히 hardening되는 경로다. |
| QEMU multi-range VFIO DMA-BUF와 virtio-gpu | Vivek Kasireddy | v12 진행 중 | [QEMU v12 series](https://patchew.org/QEMU/20260319052023.2088685-1-vivek.kasireddy%40intel.com/) | #38의 후속 설계 참고 | UPCIE의 직접 dependency는 아니지만 여러 IOV를 하나의 VFIO DMA-BUF로 만들며 range 합산, mmap, backing 교체와 cleanup을 다룬다. #38의 단일 range 가정을 확장할 때 비교해야 한다. |
...
| DMA-BUF mapping types and PAL | Jason Gunthorpe | RFC v1, 커널 내부 ABI 분쟁 중 | [mapping types/PAL RFC v1](https://lore.kernel.org/all/0-v1-b5cab63049c0+191af-dmabuf_map_type_jgg@nvidia.com/), [PAL patch 스레드](https://patchwork.kernel.org/project/dri-devel/patch/21-v1-b5cab63049c0+191af-dmabuf_map_type_jgg@nvidia.com/) | #38의 일반화 및 #40 CUDA 경로의 핵심 후보 | exporter가 DMA 주소 형태와 물리 주소 목록을 명시하게 해 VFIO BAR 한 종류에 묶인 경로를 일반 DMA-BUF로 넓힐 가능성이 있다. 그러나 revoke, lifetime, 보안과 NVIDIA exporter가 해결돼야 하고, 2026-04~05 스레드에서 dma-buf maintainer가 importer에 PA 목록을 노출하는 형태를 반대해 커널 내부 인터페이스를 재설계 중이다(§5.4). |
| IOMMUFD no-IOMMU support | Jacob Pan, Jason Gunthorpe | v10 (2026-07), 고속 iteration 중 | [no-IOMMU cdev v10](https://lore.kernel.org/all/cover.1783360051.git.jacob.pan@linux.microsoft.com/) | #40 UIO/LUT 경로의 대체 또는 정리 후보 | `/proc/pagemap`과 자체 physical LUT 대신 IOMMUFD 객체 모델 및 `GET_PA`류 ABI를 사용할 수 있다. API는 통일해도 DMA isolation이 생기는 것은 아니다. |
...
해 NVMe READ 결과를 그 region으로 보내는 흐름이다. 여기에는 서로 다른
두 upstream 기능이 필요하다.

```text
GPU bound to vfio-pci
  → VFIO_DEVICE_FEATURE_DMA_BUF
  → revocable DMA-BUF fd             [VFIO exporter]
  → IOMMU_IOAS_MAP_FILE              [IOMMUFD importer]
  → GPU BAR에 대응하는 IOVA
...
첫 번째는
[Vivek Kasireddy의 v3 series](https://lore.kernel.org/all/20250307052248.405803-1-vivek.kasireddy@intel.com/)가
Jason Gunthorpe의 초기 작업을 되살리고 Leon Romanovsky가 후속 정리한
[VFIO PCI MMIO DMA-BUF exporter](https://github.com/torvalds/linux/commit/5d74781ebc86c5fa9e9d6934024c505412de9b52)다.
BAR range를 fd로 만들고 VFIO device close/reset 때 접근을 revoke한다.

두 번째는 Jason Gunthorpe의
[IOMMUFD DMA-BUF importer patch](https://lore.kernel.org/all/8-v2-b2c110338e3f+5c2-iommufd_dmabuf_jgg@nvidia.com/)다.
exporter가 만든 fd를 `IOMMU_IOAS_MAP_FILE`로 받아 IOAS에 IOVA를 만든다.
따라서 exporter가 없으면 import할 fd가 없고, importer가 없으면 export된
BAR를 NVMe와 같은 IOAS에 배치할 수 없다. #38은 이 두 mainline ABI를
처음부터 끝까지 조합한 PCI region P2P proof of concept다.
...
Vivek의
[QEMU multi-range VFIO DMA-BUF v12](https://patchew.org/QEMU/20260319052023.2088685-1-vivek.kasireddy%40intel.com/)도
#38의 후속 분석에 가치가 있다. 이 series의 직접 목적은 NVMe가 아니라
VFIO device-local memory를 virtio-gpu blob backing으로 전달하는 것이지만,
여러 IOV를 하나의 DMA-BUF로 만들면서 다음 문제를 실제 consumer에서
처리한다.

- 모든 IOV가 같은 VFIO region에 속하는지 확인
- range 총합과 DMA-BUF 크기가 일치하는지 확인
- DMA-BUF mmap 및 실패 시 fd/object 정리
- backing이 바뀔 때 기존 DMA-BUF를 파괴하고 재생성

Simon #38은 단일 BAR range와 산술 IOVA 변환을 전제로 하므로 당장 이 QEMU
코드를 필요로 하지는 않는다. 하지만 fragmented VRAM 또는 여러 range를
지원하려면 같은 검증과 lifetime 문제가 생긴다. 반면 Vivek의
[virtio-gpu 외부 DMA-BUF import](https://lore.kernel.org/dri-devel/20241126031643.3490496-1-vivek.kasireddy@intel.com/)는
VM scanout/blob 경로이므로 현재 host P2P의 직접 dependency는 아니다.
...
2. VFIO exporter로 PCI region fd를 만들고 IOMMUFD importer로 IOAS에
   mapping하는 #38 경로
...
### 5.4 PAL의 커널 내부 설계 분쟁과 upcie에 주는 의미

2026-04-13부터 05-11까지
[PAL patch 스레드](https://patchwork.kernel.org/project/dri-devel/patch/21-v1-b5cab63049c0+191af-dmabuf_map_type_jgg@nvidia.com/)에서
dma-buf maintainer(Christian König)와 Jason Gunthorpe 사이에 PAL의
핵심 형태에 대한 분쟁이 진행됐고 미해결 상태다.

- König는 importer가 물리 주소 목록을 받는 인터페이스 자체를
  "NO-GO"로 반대했다. scatterlist 시절 importer의 PA 오남용 전례가
  근거다. 대안으로 exporter가 직접 `iommu_domain`에 map하는 역전 모델
  (`dma_buf_map_attachment_iommu(attachment, domain, iova, offset,
  size)`)을 제안했다.
- Gunthorpe는 importer를 `EXPORT_SYMBOL_FOR_MODULES()`로 iommufd에만
  잠갔다는 점과, "CPU IOMMU 하드웨어는 물리 주소로만 동작한다"는 점으로
  방어했다. 관심 경로로 (1) VFIO→KVM FD import, (2) **GPU/RDMA 드라이버를
  IOMMUFD로 — "SPDK/DPDK류 userspace 드라이버가 GPU 메모리로 작업하도록
  지원, 인기 주제가 되는 중"** 두 가지를 명시했다. 두 번째가 정확히
  upcie와 Simon PR 스택의 use case다.
- 절충안으로 dmabuf core만 exporter의 주소 데이터와 importer의
  `iommu_domain`을 모두 보는 core 중재 모델이 논의됐으나, KVM은 주소
  공간 관리자이자 mapper라 넘겨줄 domain이 없다는 문제(Xu Yilun 지적)로
  스레드가 멈춰 있다.

upcie 관점의 결론은 세 가지다. 첫째, RFC v1 이후 v2가 나오지 않는
이유는 리뷰 지연이 아니라 커널 내부 인터페이스의 재설계이며, PAL 일정
리스크는 그만큼 크다. 둘째, 분쟁 대상은 커널 내부 exporter↔importer
인터페이스이지 userspace ABI가 아니다 — 어느 설계가 이겨도 userspace는
dma-buf fd를 `IOMMU_IOAS_MAP_FILE`에 넘기는 형태를 유지하므로, #38의
흐름과 `dmamem` 추상화는 양쪽 설계 모두에서 유효하다. 셋째, upcie는
Gunthorpe가 근거로 든 use case의 동작하는 PoC를 이미 가지고 있으므로,
이 스레드에 실사용 소비자 데이터포인트로 회신하는 것이 upstream 논의에
실질적인 기여가 될 수 있다.
...
- VFIO exporter의 range/revoke와 IOMMUFD importer의 unmap ordering
...
- PAL: #38의 VFIO BAR 전용 exporter/importer 조합을 일반 DMA-BUF
  mapping으로 확장. 단 §5.4의 설계 분쟁으로 일정 리스크가 크다
...
2. #38의 VFIO exporter/importer lifetime, range, P2P topology와 queue lifetime
3. #39의 type1/IOMMUFD 선택 및 실패 후 fallback
4. #40의 LUT-to-PRP 변환, pinning, address validity와 no-IOMMU 위험
5. Vivek의 QEMU multi-range consumer가 #38 확장에 주는 range/lifetime 교훈
6. PAL RFC와 NVIDIA exporter가 #38/#40의 양분된 경로를 통합할 수 있는지
7. no-IOMMU RFC가 #40의 실험 경로를 더 명확한 ABI로 옮길 수 있는지
...
- [Vivek's VFIO PCI MMIO DMA-BUF exporter v3](https://lore.kernel.org/all/20250307052248.405803-1-vivek.kasireddy@intel.com/)
- [Merged VFIO PCI MMIO DMA-BUF exporter](https://github.com/torvalds/linux/commit/5d74781ebc86c5fa9e9d6934024c505412de9b52)
