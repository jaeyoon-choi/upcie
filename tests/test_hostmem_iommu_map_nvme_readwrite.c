...
/**
 * Control experiment for option-A: host RAM mapped at a high IOVA
 * ==============================================================
 *
...
/* Same window as the cudamem test: above 64 GiB RAM, below 512 GiB aperture. */
...
iommu_map_data_heap(struct nvme *nvme, const char *bdf, struct hostmem_heap *data)
{
	...
	err = upcie_dmabuf_importer_iommu_map(nvme->importer_fd, bdf, -1,
...
nvme_cleanup(struct nvme *nvme)
{
	...
	if (nvme->iommu_handle) {
		upcie_dmabuf_importer_unmap_handle(nvme->importer_fd, nvme->iommu_handle);
		nvme->iommu_handle = 0;
	}
	if (nvme->importer_fd >= 0) {
		upcie_dmabuf_importer_close(nvme->importer_fd);
